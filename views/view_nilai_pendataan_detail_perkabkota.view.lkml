...
view: view_nilai_pendataan_detail_perkabkota {
  derived_table: {
    sql: select kode_provinsi,province as provinsi,kode_kabkota, city as nama_kabkota,jenjang,round(sum(student_graduation)/count(nsm),2) as total_status_gradution,round(sum(avg_student)/count(nsm),2) as total_rasiosiswa_perrombel,round(sum(rasioguru)/count(nsm),2) as total_rasio_guru,round(sum(gurubersertifikat
       ...
       group by kode_provinsi,province,kode_kabkota, city,jenjang ;;
  }
