...
view: view_nilai_akhir_kabkota {
  derived_table: {
    sql: SELECT
                    ...
                    )*10)/count(aspek_pendataan_permadrasah.nsm),2) AS NilaiAkhir_pendataan,
                    round((sum(aspek_tatakelola.avg_lpj + aspek_tatakelola.skor_akhir) / 2)/count(aspek_tatakelola.nsm),2) as NilaiAkhir_tatakelola,
                    case when sum(total_row) = 0 then 0 else round(sum(total_skor)/sum(total_row),2) end as NilaiAkhir_guru,
                    case when sum(total_row_siswa) = 0 then 0 else round(sum(total_skor_siswa)/sum(total_row_siswa),2) end as NilaiAkhir_siswa
...
cast(aspek_siswa_permadrasah.nsm as string) = aspek_pendataan_permadrasah.nsm and aspek_siswa_permadrasah.tahun=aspek_pendataan_permadrasah.tahun
  ...
  dimension: nilai_akhir_pendataan {
    type: number
    sql: ${TABLE}.NilaiAkhir_pendataan ;;
  }

  dimension: nilai_akhir_tatakelola {
    type: number
    sql: ${TABLE}.NilaiAkhir_tatakelola ;;
  }

  dimension: nilai_akhir_guru {
    type: number
    sql: ${TABLE}.NilaiAkhir_guru ;;
  }
  dimension: nilai_akhir_siswa {
    type: number
    sql: ${TABLE}.NilaiAkhir_siswa ;;
  }
  ...
  nama_kabkota,
  jenjang,
  nilai_akhir_pendataan,
  nilai_akhir_tatakelola,
  nilai_akhir_guru,
  nilai_akhir_siswa
    ]
  }
