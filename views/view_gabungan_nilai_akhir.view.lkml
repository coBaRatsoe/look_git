...
view: view_gabungan_nilai_akhir {
  derived_table: {
    sql: SELECT
                          ...
                          )*10)/count(aspek_pendataan_permadrasah.nsm),2) AS view_nilai_akhir,
                          round((sum(aspek_tatakelola.avg_lpj + aspek_tatakelola.skor_akhir) / 2)/count(aspek_tatakelola.nsm),2) as view_nilai_akhir_tata_kelola,
                          case when sum(total_row) = 0 then 0 else round(sum(total_skor)/sum(total_row),2) end as view_nilai_akhir_guru,
                          case when sum(total_row_siswa) = 0 then 0 else round(sum(total_skor_siswa)/sum(total_row_siswa),2) end as view_nilai_akhir_siswa
  ...
  dimension: view_nilai_akhir {
    type: number
    sql: ${TABLE}.view_nilai_akhir ;;
  }

  dimension: view_nilai_akhir_tata_kelola {
    type: number
    sql: ${TABLE}.view_nilai_akhir_tata_kelola ;;
  }

  dimension: view_nilai_akhir_guru {
    type: number
    sql: ${TABLE}.view_nilai_akhir_guru ;;
  }
  ...
  view_nilai_akhir,
  view_nilai_akhir_tata_kelola,
  view_nilai_akhir_guru,
  view_nilai_akhir_siswa
    ]
  }
}
